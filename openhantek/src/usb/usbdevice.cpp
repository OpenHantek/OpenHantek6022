...
USBDevice::USBDevice( DSOModel *model, libusb_device *device, unsigned findIteration )
    : model( model ), device( device ), findIteration( findIteration ),
...
bool USBDevice::connectDevice( QString &errorMessage ) {
    ...
    if ( errorCode != LIBUSB_SUCCESS ) {
        handle = nullptr;
        errorMessage =
            QCoreApplication::translate( "", "Couldn't open device: %1" ).arg( libUsbErrorString( errorCode ) );
        return false;
    }
...
int USBDevice::bulkReadMulti( unsigned char *data, unsigned length, int attempts ) {
    ...
    return this->bulkTransfer( HANTEK_EP_IN, data, length, attempts,
                               HANTEK_TIMEOUT_MULTI * length / this->inPacketLength );
#else
