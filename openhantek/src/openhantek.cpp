...
OpenHantekMainWindow::OpenHantekMainWindow(QWidget *parent,
...
void OpenHantekMainWindow::createActions() {
  ...
  this->saveAsAction->setStatusTip(
  ...
  connect(this->printAction, SIGNAL(triggered()), this->dsoWidget,
          SLOT(print()));

  this->exportAsAction =
      new QAction(QIcon(":actions/export-as.png"), tr("&Export as..."), this);
  this->exportAsAction->setShortcut(tr("Ctrl+E"));
  this->exportAsAction->setStatusTip(
      tr("Export the oscilloscope data to a file"));
  connect(this->exportAsAction, SIGNAL(triggered()), this->dsoWidget,
          SLOT(exportAs()));
  ...
  this->digitalPhosphorAction = new QAction(
  ...
  connect(this->digitalPhosphorAction, SIGNAL(toggled(bool)), this,
  ...
  connect(this->zoomAction, SIGNAL(toggled(bool)), this->dsoWidget,
...
#ifdef DEBUG
  connect(this->commandAction, SIGNAL(triggered()), this->commandEdit,
          SLOT(show()));
  connect(this->commandAction, SIGNAL(triggered()), this->commandEdit,
          SLOT(setFocus()));
  connect(this->commandEdit, SIGNAL(returnPressed()), this,
          SLOT(sendCommand()));
#endif
}
...
void OpenHantekMainWindow::createDockWindows() {
  ...
  this->triggerDock = new TriggerDock(
      this->settings, this->dsoControl->getSpecialTriggerSources());
...
void OpenHantekMainWindow::connectSignals() {
  ...
  connect(this->dsoControl, SIGNAL(statusMessage(QString, int)),
          this->statusBar(), SLOT(showMessage(QString, int)));
  connect(this->dsoControl,
  ...
  connect(this->horizontalDock, SIGNAL(samplerateChanged(double)), this,
          SLOT(samplerateSelected()));
  connect(this->horizontalDock, SIGNAL(timebaseChanged(double)), this,
          SLOT(timebaseSelected()));
  connect(this->horizontalDock, SIGNAL(frequencybaseChanged(double)),
          this->dsoWidget, SLOT(updateFrequencybase(double)));
  connect(this->horizontalDock, SIGNAL(recordLengthChanged(unsigned long)),
          this, SLOT(recordLengthSelected(unsigned long)));
  // connect(this->horizontalDock, SIGNAL(formatChanged(HorizontalFormat)),
  // this->dsoWidget, SLOT(horizontalFormatChanged(HorizontalFormat)));

  connect(this->triggerDock, SIGNAL(modeChanged(Dso::TriggerMode)),
          this->dsoControl, SLOT(setTriggerMode(Dso::TriggerMode)));
  connect(this->triggerDock, SIGNAL(modeChanged(Dso::TriggerMode)),
          this->dsoWidget, SLOT(updateTriggerMode()));
  connect(this->triggerDock, SIGNAL(sourceChanged(bool, unsigned int)),
          this->dsoControl, SLOT(setTriggerSource(bool, unsigned int)));
  connect(this->triggerDock, SIGNAL(sourceChanged(bool, unsigned int)),
          this->dsoWidget, SLOT(updateTriggerSource()));
  connect(this->triggerDock, SIGNAL(slopeChanged(Dso::Slope)), this->dsoControl,
          SLOT(setTriggerSlope(Dso::Slope)));
  connect(this->triggerDock, SIGNAL(slopeChanged(Dso::Slope)), this->dsoWidget,
          SLOT(updateTriggerSlope()));
  connect(this->dsoWidget, SIGNAL(triggerPositionChanged(double)),
          this->dsoControl, SLOT(setPretriggerPosition(double)));
  connect(this->dsoWidget, SIGNAL(triggerLevelChanged(unsigned int, double)),
          this->dsoControl, SLOT(setTriggerLevel(unsigned int, double)));

  connect(this->voltageDock, SIGNAL(usedChanged(unsigned int, bool)), this,
          SLOT(updateUsed(unsigned int)));
  connect(this->voltageDock, SIGNAL(usedChanged(unsigned int, bool)),
          this->dsoWidget, SLOT(updateVoltageUsed(unsigned int, bool)));
  connect(this->voltageDock,
          SIGNAL(couplingChanged(unsigned int, Dso::Coupling)),
          this->dsoControl, SLOT(setCoupling(unsigned int, Dso::Coupling)));
  connect(this->voltageDock,
          SIGNAL(couplingChanged(unsigned int, Dso::Coupling)), this->dsoWidget,
          SLOT(updateVoltageCoupling(unsigned int)));
  connect(this->voltageDock, SIGNAL(modeChanged(Dso::MathMode)),
          this->dsoWidget, SLOT(updateMathMode()));
  connect(this->voltageDock, SIGNAL(gainChanged(unsigned int, double)), this,
          SLOT(updateVoltageGain(unsigned int)));
  connect(this->voltageDock, SIGNAL(gainChanged(unsigned int, double)),
          this->dsoWidget, SLOT(updateVoltageGain(unsigned int)));
  connect(this->dsoWidget, SIGNAL(offsetChanged(unsigned int, double)), this,
          SLOT(updateOffset(unsigned int)));

  connect(this->spectrumDock, SIGNAL(usedChanged(unsigned int, bool)), this,
          SLOT(updateUsed(unsigned int)));
  connect(this->spectrumDock, SIGNAL(usedChanged(unsigned int, bool)),
          this->dsoWidget, SLOT(updateSpectrumUsed(unsigned int, bool)));
  connect(this->spectrumDock, SIGNAL(magnitudeChanged(unsigned int, double)),
          this->dsoWidget, SLOT(updateSpectrumMagnitude(unsigned int)));
  ...
  // connect(this->dsoControl, SIGNAL(recordLengthChanged(unsigned long)), this,
  // SLOT(recordLengthChanged()));
  connect(this->dsoControl, SIGNAL(recordTimeChanged(double)), this,
          SLOT(recordTimeChanged(double)));
  connect(this->dsoControl, SIGNAL(samplerateChanged(double)), this,
  ...
  connect(this->dsoControl, SIGNAL(samplerateLimitsChanged(double, double)),
          this->horizontalDock, SLOT(samplerateLimitsChanged(double, double)));
  connect(this->dsoControl, SIGNAL(samplerateSet(int, QList<double>)),
          this->horizontalDock, SLOT(samplerateSet(int, QList<double>)));
}

/// \brief Initialize the device with the current settings.
void OpenHantekMainWindow::initializeDevice() {
  for (unsigned int channel = 0;
       channel < this->settings->scope.physicalChannels; ++channel) {
    this->dsoControl->setCoupling(
        channel, (Dso::Coupling)this->settings->scope.voltage[channel].misc);
    this->updateVoltageGain(channel);
    this->updateOffset(channel);
    this->dsoControl->setTriggerLevel(
...
void OpenHantekMainWindow::started() {
  ...
  disconnect(this->startStopAction, SIGNAL(triggered()), this->dsoControl,
             SLOT(startSampling()));
  connect(this->startStopAction, SIGNAL(triggered()), this->dsoControl,
          SLOT(stopSampling()));
}
...
void OpenHantekMainWindow::digitalPhosphor(bool enabled) {
  ...
  if (this->settings->view.digitalPhosphor)
    this->digitalPhosphorAction->setStatusTip(
...
void OpenHantekMainWindow::updateOffset(unsigned int channel) {
  ...
  this->dsoControl->setOffset(
      channel,
      (this->settings->scope.voltage[channel].offset / DIVS_VOLTAGE) + 0.5);
}
...
void OpenHantekMainWindow::updateUsed(unsigned int channel) {
  ...
  bool mathUsed =
      this->settings->scope.voltage[this->settings->scope.physicalChannels]
          .used |
      this->settings->scope.spectrum[this->settings->scope.physicalChannels]
          .used;

  // Normal channel, check if voltage/spectrum or math channel is used
  if (channel < this->settings->scope.physicalChannels)
    this->dsoControl->setChannelUsed(
        channel, mathUsed | this->settings->scope.voltage[channel].used |
  ...
  // Math channel, update all channels
    ...
    for (unsigned int channelCounter = 0;
         channelCounter < this->settings->scope.physicalChannels;
         ++channelCounter)
      this->dsoControl->setChannelUsed(
          channelCounter,
          mathUsed | this->settings->scope.voltage[channelCounter].used |
              this->settings->scope.spectrum[channelCounter].used);
  }
}
...
void OpenHantekMainWindow::updateVoltageGain(unsigned int channel) {
  ...
  this->dsoControl->setGain(
      channel, this->settings->scope.voltage[channel].gain * DIVS_VOLTAGE);
}
