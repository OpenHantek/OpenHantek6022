...
QString valueToString(double value, Unit unit, int precision) {
  ...
  case UNIT_VOLTS: {
    ...
    if (value < 1e-3)
      return QApplication::tr("%L1 µV").arg(
          value / 1e-6, 0, format,
          (precision <= 0) ? precision
                           : qBound(0, precision - 7 - logarithm, precision));
    else if (value < 1.0)
      return QApplication::tr("%L1 mV").arg(
          value / 1e-3, 0, format,
  ...
  case UNIT_DECIBEL:
    ...
    return QApplication::tr("%L1 dB").arg(
        ...
        (precision <= 0)
            ? precision
            : qBound(0, precision - 1 - (int)floor(log10(fabs(value))),
                     precision));
  ...
  case UNIT_SECONDS:
    ...
    if (value < 1e-9)
      return QApplication::tr("%L1 ps").arg(
          ...
          (precision <= 0)
              ? precision
              : qBound(0, precision - 13 - (int)floor(log10(fabs(value))),
                       precision));
    else if (value < 1e-6)
      return QApplication::tr("%L1 ns").arg(
          value / 1e-9, 0, format,
          (precision <= 0) ? precision
                           : (precision - 10 - (int)floor(log10(fabs(value)))));
    else if (value < 1e-3)
      return QApplication::tr("%L1 µs").arg(
          value / 1e-6, 0, format,
          (precision <= 0) ? precision
                           : (precision - 7 - (int)floor(log10(fabs(value)))));
    else if (value < 1.0)
      return QApplication::tr("%L1 ms").arg(
          value / 1e-3, 0, format,
          (precision <= 0) ? precision
      ...
      return QApplication::tr("%L1 s").arg(
          value, 0, format,
          (precision <= 0) ? precision
                           : (precision - 1 - (int)floor(log10(fabs(value)))));
    else if (value < 3600)
      return QApplication::tr("%L1 min").arg(
          value / 60, 0, format,
          (precision <= 0) ? precision
                           : (precision - 1 - (int)floor(log10(value / 60))));
    else
      return QApplication::tr("%L1 h").arg(
          value / 3600, 0, format,
          (precision <= 0)
              ? precision
              : qMax(0, precision - 1 - (int)floor(log10(value / 3600))));
  ...
  case UNIT_HERTZ: {
    ...
    if (value < 1e3)
      return QApplication::tr("%L1 Hz").arg(
          value, 0, format,
          (precision <= 0) ? precision
                           : qBound(0, precision - 1 - logarithm, precision));
    else if (value < 1e6)
      return QApplication::tr("%L1 kHz").arg(
          value / 1e3, 0, format,
          (precision <= 0) ? precision : precision + 2 - logarithm);
    else if (value < 1e9)
      return QApplication::tr("%L1 MHz").arg(
          value / 1e6, 0, format,
          (precision <= 0) ? precision : precision + 5 - logarithm);
    else
      return QApplication::tr("%L1 GHz").arg(
          value / 1e9, 0, format,
  ...
  case UNIT_SAMPLES: {
    ...
    if (value < 1e3)
      ...
      return QApplication::tr("%L1 kS").arg(
          value / 1e3, 0, format,
          (precision <= 0) ? precision : precision + 2 - logarithm);
    else if (value < 1e9)
      return QApplication::tr("%L1 MS").arg(
      ...
      return QApplication::tr("%L1 GS").arg(
          value / 1e9, 0, format,
          (precision <= 0) ? precision : qMax(0, precision + 8 - logarithm));
  }
  default:
...
double stringToValue(const QString &text, Unit unit, bool *ok) {
  ...
  for (; valueSize < text.size(); ++valueSize) {
    ...
    } else if (character == locale.decimalPoint() && decimalFound == false &&
               exponentFound == false) {
      decimalFound = true;
