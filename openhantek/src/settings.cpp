...
void DsoSettings::setChannelCount(unsigned int channels) {
  ...
  for (int channel = this->scope.spectrum.count() - 2; channel >= (int)channels;
       channel--)
    this->scope.spectrum.removeAt(channel);
  for (int channel = this->scope.voltage.count() - 2; channel >= (int)channels;
       channel--)
  ...
  for (int channel = 0; channel < (int)channels; ++channel) {
    ...
    if (this->view.color.screen.voltage.count() <= channel + 1)
      this->view.color.screen.voltage.insert(
          channel, QColor::fromHsv(channel * 60, 0xff, 0xff));
    if (this->view.color.screen.spectrum.count() <= channel + 1)
      this->view.color.screen.spectrum.insert(
          channel, this->view.color.screen.voltage[channel].lighter());
    // Print
    if (this->view.color.print.voltage.count() <= channel + 1)
      this->view.color.print.voltage.insert(
          channel, this->view.color.screen.voltage[channel].darker(120));
    if (this->view.color.print.spectrum.count() <= channel + 1)
      this->view.color.print.spectrum.insert(
          channel, this->view.color.screen.voltage[channel].darker());
  }
  ...
  if (this->view.color.screen.spectrum.count() <= (int)channels)
    this->view.color.screen.spectrum.append(
        this->view.color.screen.voltage[channels].lighter());
  if (this->view.color.print.voltage.count() <= (int)channels)
    this->view.color.print.voltage.append(
        this->view.color.screen.voltage[channels]);
  if (this->view.color.print.spectrum.count() <= (int)channels)
    this->view.color.print.spectrum.append(
        this->view.color.print.voltage[channels].darker());
}
...
int DsoSettings::load(const QString &fileName) {
  ...
  if (settingsLoader->contains("format"))
    this->scope.horizontal.format =
        (Dso::GraphFormat)settingsLoader->value("format").toInt();
  if (settingsLoader->contains("frequencybase"))
    this->scope.horizontal.frequencybase =
        settingsLoader->value("frequencybase").toDouble();
  for (int marker = 0; marker < 2; ++marker) {
    QString name;
    name = QString("marker%1").arg(marker);
    if (settingsLoader->contains(name))
      this->scope.horizontal.marker[marker] =
          settingsLoader->value(name).toDouble();
  }
  if (settingsLoader->contains("timebase"))
    this->scope.horizontal.timebase =
        settingsLoader->value("timebase").toDouble();
  if (settingsLoader->contains("recordLength"))
    this->scope.horizontal.recordLength =
        settingsLoader->value("recordLength").toUInt();
  if (settingsLoader->contains("samplerate"))
    this->scope.horizontal.samplerate =
        settingsLoader->value("samplerate").toDouble();
  if (settingsLoader->contains("samplerateSet"))
    this->scope.horizontal.samplerateSet =
        settingsLoader->value("samplerateSet").toBool();
  settingsLoader->endGroup();
  // Trigger
  ...
  if (settingsLoader->contains("mode"))
    this->scope.trigger.mode =
        (Dso::TriggerMode)settingsLoader->value("mode").toInt();
  ...
  if (settingsLoader->contains("slope"))
    this->scope.trigger.slope =
        (Dso::Slope)settingsLoader->value("slope").toInt();
  ...
  for (int channel = 0; channel < this->scope.spectrum.count(); ++channel) {
    ...
    if (settingsLoader->contains("magnitude"))
      this->scope.spectrum[channel].magnitude =
          settingsLoader->value("magnitude").toDouble();
    if (settingsLoader->contains("offset"))
      this->scope.spectrum[channel].offset =
          settingsLoader->value("offset").toDouble();
    if (settingsLoader->contains("used"))
      this->scope.spectrum[channel].used =
          settingsLoader->value("used").toBool();
    settingsLoader->endGroup();
  }
  // Vertical axis
  for (int channel = 0; channel < this->scope.voltage.count(); ++channel) {
    ...
    if (settingsLoader->contains("gain"))
      this->scope.voltage[channel].gain =
          settingsLoader->value("gain").toDouble();
    if (settingsLoader->contains("misc"))
      this->scope.voltage[channel].misc = settingsLoader->value("misc").toInt();
    if (settingsLoader->contains("offset"))
      this->scope.voltage[channel].offset =
          settingsLoader->value("offset").toDouble();
    if (settingsLoader->contains("trigger"))
      this->scope.voltage[channel].trigger =
          settingsLoader->value("trigger").toDouble();
    if (settingsLoader->contains("used"))
      this->scope.voltage[channel].used =
          settingsLoader->value("used").toBool();
    settingsLoader->endGroup();
  }
  if (settingsLoader->contains("spectrumLimit"))
    this->scope.spectrumLimit =
        settingsLoader->value("spectrumLimit").toDouble();
  if (settingsLoader->contains("spectrumReference"))
    this->scope.spectrumReference =
        settingsLoader->value("spectrumReference").toDouble();
  if (settingsLoader->contains("spectrumWindow"))
    this->scope.spectrumWindow =
        (Dso::WindowFunction)settingsLoader->value("spectrumWindow").toInt();
  settingsLoader->endGroup();
  ...
  if (settingsLoader->contains("digitalPhosphor"))
    this->view.digitalPhosphor =
        settingsLoader->value("digitalPhosphor").toBool();
  if (settingsLoader->contains("interpolation"))
    this->view.interpolation =
...
int DsoSettings::save(const QString &fileName) {
  ...
  settingsSaver->setValue("frequencybase",
                          this->scope.horizontal.frequencybase);
  for (int marker = 0; marker < 2; ++marker)
    settingsSaver->setValue(QString("marker%1").arg(marker),
                            this->scope.horizontal.marker[marker]);
  ...
  settingsSaver->setValue("samplerateSet",
                          this->scope.horizontal.samplerateSet);
  settingsSaver->endGroup();
  // Trigger
  ...
  for (int channel = 0; channel < this->scope.spectrum.count(); ++channel) {
    ...
    settingsSaver->setValue("magnitude",
                            this->scope.spectrum[channel].magnitude);
  ...
  if (complete) {
    ...
    for (int mode = 0; mode < 2; ++mode) {
      ...
      for (int channel = 0; channel < this->scope.spectrum.count(); ++channel)
        settingsSaver->setValue(QString("spectrum%1").arg(channel),
                                colors->spectrum[channel]);
      settingsSaver->setValue("text", colors->text);
      for (int channel = 0; channel < this->scope.voltage.count(); ++channel)
        settingsSaver->setValue(QString("voltage%1").arg(channel),
                                colors->voltage[channel]);
      settingsSaver->endGroup();
    }
    settingsSaver->endGroup();
  }
