...
#define HANTEK_TIMEOUT_MULTI                                                   \
    100                     ///< Timeout for multi packet USB transfers in ms
#define HANTEK_ATTEMPTS 3 ///< The number of transfer attempts
#define HANTEK_ATTEMPTS_MULTI                                                  \
    1 ///< The number of multi packet transfer attempts

/// \brief This class handles the USB communication with an usb device that has one in and one out endpoint.
...
public:
    ...
    int bulkCommand(DataArray<unsigned char> *command,
                    int attempts = HANTEK_ATTEMPTS);
