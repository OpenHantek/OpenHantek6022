...
std::list<std::unique_ptr<USBDevice>> FindDevices::findDevices() {
    ...
    if (noAccessDeviceCount == deviceCount) {
        ...
        errorMessage = QCoreApplication::translate("","Please make sure to have read/write access to your usb device. On linux you need to install the correct udev file for example.");
    }
...
const QString &FindDevices::getErrorMessage() const
{
    return errorMessage;
}

bool FindDevices::allDevicesNoAccessError() const
{
    return noAccessDevices;
}
