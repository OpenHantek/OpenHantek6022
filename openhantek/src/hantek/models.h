...
class DSOModel {
  public:
    ...
    DSOModel(Model model, long vendorID, long productID, long vendorIDnoFirmware, long productIDnoFirmware,
        ...
        : uniqueModelID(model), vendorID(vendorID), productID(productID), vendorIDnoFirmware(vendorIDnoFirmware),
          productIDnoFirmware(productIDnoFirmware), firmwareToken(firmwareToken), name(name) {}
};
