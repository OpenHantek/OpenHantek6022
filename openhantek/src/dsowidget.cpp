...
DsoWidget::DsoWidget( DsoSettingsScope *scope, DsoSettingsView *view, const Dso::ControlSpecification *spec,
    ...
    : QWidget( parent, flags ), scope( scope ), view( view ), spec( spec ),
      mainScope( GlScope::createNormal( scope, view ) ), zoomScope( GlScope::createZoomed( scope, view ) ) {
...
void DsoWidget::setupSliders( DsoWidget::Sliders &sliders ) {
    ...
    for ( ChannelID channel = 0; channel < scope->voltage.size(); ++channel ) {
        sliders.voltageOffsetSlider->addSlider( scope->spectrum[ channel ].name,
                                                int( scope->voltage.size() + channel ) );
        ...
        sliders.voltageOffsetSlider->setValue( int( scope->voltage.size() + channel ),
...
void DsoWidget::adaptTriggerLevelSlider( DsoWidget::Sliders &sliders, ChannelID channel ) {
    ...
    sliders.triggerLevelSlider->setLimits(
...
void DsoWidget::updateTriggerSource() {
    ...
    for ( ChannelID channel = 0; channel < spec->channels; ++channel ) {
        QColor color = ( channel == scope->trigger.source ) ? view->screen.voltage[ channel ]
                                                            : view->screen.voltage[ channel ].darker();
