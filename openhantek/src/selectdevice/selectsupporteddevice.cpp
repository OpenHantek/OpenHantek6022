    ...
    connect( ui->buttonBox, &QDialogButtonBox::helpRequested, []() {
        ...
        if ( userManual.exists() )
            ...
            QDesktopServices::openUrl( QUrl(
                "https://github.com/OpenHantek/OpenHantek6022/blob/master/docs/OpenHantek6022_User_Manual.pdf" ) );
    } );
#ifdef NEW_DEVICE_FROM_EXISTING_DIALOG
...
std::unique_ptr<USBDevice> SelectSupportedDevice::showSelectDeviceModal( libusb_context *context ) {
    ...
    std::unique_ptr<DevicesListModel> model =
        std::unique_ptr<DevicesListModel>( new DevicesListModel( findDevices.get() ) );
                 ...
                 if ( ui->cmbDevices->currentData( Qt::UserRole + 1 ).toBool() ) { // canConnect
                     ...
                     ui->labelReadyState->setText(
                         tr( "<p><br/><b>The device is ready for use.</b></p><p>Please observe the "
                             "<a "
                             "href='https://github.com/OpenHantek/OpenHantek6022/blob/master/docs/"
                             "OpenHantek6022_User_Manual.pdf'>"
                     ...
                     if ( ui->cmbDevices->currentData( Qt::UserRole + 2 ).toBool() ) { // needFirmware
                         ui->labelReadyState->setText( tr( "<p>Upload in progress ...</p>"
                                                           "<p><b>If the upload takes more than 30 s, please close "
                                                           "this window <br/>and restart the program!</b></p>" ) );
    ...
    QString messageNoDevices =
        tr( "<p>OpenHantek6022 is searching for compatible devices ...</p>"
    ...
    if ( !libRules.exists() && !etcRules.exists() ) {
        messageNoDevices += tr( "<p>Please make sure you have copied the udev rules file to <b>%1</b> for correct USB "
                                "access permissions.</p>" )
                                .arg( libRules.fileName() );
    }
#endif
    messageNoDevices += tr(
        "<p>Visit the build and run instruction "
