...
void SiSpinBox::stepBy(int steps) {
  ...
  if (!this->steppedTo) { // No step done directly before this one, so we need
    ...
    int stepsFully = (this->mode == 0)
                         ? (int)floor(log(this->value() / this->steps.first()) /
                                      log(stepsSpan))
                         : 0;
    // And now the remaining multiple
    double stepMultiple = (this->mode == 0)
                              ? this->value() / pow(stepsSpan, stepsFully)
    ...
    if (remainingSteps >
        0) // Shouldn't happen, but double may have rounding errors
      --remainingSteps;
  ...
  for (int i = 0; i != steps; i += subStep) {
    ...
    if (!this->mode) {
      ...
      if (stepsId < 0)
        stepsId += stepsCount;
      value = pow(stepsSpan, floor((double)this->stepId / stepsCount)) *
              this->steps[stepsId];
    } else {
