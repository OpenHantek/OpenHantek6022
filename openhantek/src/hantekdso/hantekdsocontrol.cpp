...
Dso::ErrorCode HantekDsoControl::setCoupling( ChannelID channel, Dso::Coupling coupling ) {
    ...
    if ( hasCommand(
             ControlCode::CONTROL_SETCOUPLING ) ) // don't send command if it is not implemented (like on the 6022)
...
Dso::ErrorCode HantekDsoControl::retrieveChannelLevelData() {
    ...
    if ( errorCode < 0 ) {
        ...
        QString message =
            tr( "Couldn't get calibration data from oscilloscope's EEPROM. Use a config file for calibration!" );
        qWarning() << message;
...
void HantekDsoControl::convertRawDataToSamples( const std::vector<unsigned char> &rawData, unsigned activeChannels ) {
    ...
    const unsigned sampleCount =
        ( rawSampleCount > 1024 ) ? ( ( rawSampleCount - 1024 ) / 1000 - 1 ) * 1000 : rawSampleCount;
    ...
    if ( 0 == rawSampleCount || 0 == sampleCount || 0 == rawDownsampling ) { // nothing to convert
        qDebug() << "HDC::cRDTS rawSampleCount sampleCount rawDownsampling:" << rawSampleCount << sampleCount
                 << rawDownsampling;
        return;
    }
